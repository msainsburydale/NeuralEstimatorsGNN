...
pointestimator = gnnarchitecture(p) #TODO add prior: gnnarchitecture(ξ)
...
Flux.loadparams!(pointestimator,    loadbestweights(joinpath(path, "runs_pointestimator")))
Flux.loadparams!(intervalestimator, loadbestweights(joinpath(path, "runs_CIestimator")))
...
# total_time = @elapsed results = Folds.map(1:3) do k
